...
sys.path.append(FILE.parents[0].as_posix())  # add yolov5模型/ to path
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        logging.warning('DP not recommended, instead use torch.distributed.run for best DDP Multi-GPU results.\n'
                        'See Multi-GPU Tutorial at https://github.com/ultralytics/yolov5模型/issues/475 to get started.')
        model = torch.nn.DataParallel(model)

    # SyncBatchNorm
    if opt.sync_bn and cuda and RANK != -1:
        raise Exception('can not train with --sync-bn, known issue https://github.com/ultralytics/yolov5模型/issues/3998')
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default='data/car.yaml', help='dataset.yaml path')
